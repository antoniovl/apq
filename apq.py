...
def parse_mq(args):
    ...
    for line in mailq_stdout.splitlines():
        if state == MQ_STATE_DONE:
            ...
            msgs[queue_id] = msg
            state = MQ_STATE_REASON
            recipients = []
            addresses = []
            continue
        elif line.lstrip(' ')[0] == '(':
            if state != MQ_STATE_REASON and state != MQ_STATE_RCPT:
                _quit("Unexpected state for input \"{}\"".format(line))
            if state == MQ_STATE_RCPT:
            ...
            if state != MQ_STATE_RCPT:
                _quit("Expected recipient address, got \"{}\"".format(line))
...
def parse_args():
    ...
    parser.add_argument("--postfix3", default=False, type=bool,
                        help="Output compatible with 'postqueue -j' (Postfix 3.x).")
    ...
    if args.count:
        ...
        if args.yaml:
            ...
            for queue_id in msgs.keys():
                msg = msgs[queue_id]
                p3msg = convert_to_postfix3(queue_id, msg)
                print(json.dumps(p3msg, indent=2))
        else:
...
def convert_to_postfix3(queue_id, msg):
    p3m = {
        "queue_name": msg.get("status"),
        "queue_id": queue_id,
        "arrival_time": msg.get("rawdate"),
